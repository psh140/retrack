...
- [x] `FileServiceTest` (5케이스) — 파일명 null 검증, 불허 확장자 보안 검증, DB 실패 시 파일시스템 롤백, RESEARCHER 소유권 검증, ADMIN 역할 분기
- [x] `ProjectServiceTest` (3케이스) — 불허 상태 전이, 허용 전이 3가지 작업 모두 호출 verify, RESEARCHER 타인 과제 수정 권한 검증

#### 기능 개선 (2026-05-18)
- [x] `ProjectService.changeStatus()` — `userRole` 파라미터 추가, MANAGER/ADMIN은 상태 전이 규칙 없이 모든 상태로 변경 가능
- [x] `ProjectController.changeStatus()` — request attribute에서 `role` 추출하여 서비스로 전달
...
- [x] `FileService.uploadFile()` — `getOriginalFilename()` null/빈 문자열 검증 추가 (NPE 방지)
- [x] `FileService.deleteFile()` — `uploadedBy` null 체크 추가 (유저 삭제 후 파일 접근 시 NPE 방지)
- [x] `schema.sql` — `PROJECT_HISTORY.changed_by`, `BUDGET.used_by`, `FILES.uploaded_by` FK: `ON DELETE CASCADE` → `ON DELETE SET NULL` + NOT NULL 제거 (감사 기록 보존)
- [x] `schema.sql` — `NOTIFICATIONS.created_at` 컬럼 추가 (`TIMESTAMP NOT NULL DEFAULT NOW()`)
- [x] `NotificationVO` — `createdAt` 필드 추가
- [x] `NotificationMapper.xml` — `created_at` resultMap/SELECT 반영, 목록 정렬 `notification_id DESC` → `created_at DESC`
