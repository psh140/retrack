...
12단계(JUnit) 완료 후, 프론트엔드 작업 시작 전에 아래 구조로 분리한다.

```
CLAUDE.md             ← 공통 (프로젝트 개요, 기술스택, 실행방법, 협업규칙, 하네스)
backend/CLAUDE.md     ← 백엔드 개발규칙, 파일저장소/트랜잭션 설계, 개발현황, 트러블슈팅
frontend/CLAUDE.md    ← 프론트엔드 개발규칙, 기술스택 확정 내용, 개발현황
```

Claude Code는 서브디렉토리 CLAUDE.md를 자동 인식하므로, 백엔드 작업 시 루트 + backend/CLAUDE.md, 프론트엔드 작업 시 루트 + frontend/CLAUDE.md가 함께 적용된다.
